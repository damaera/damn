...
/**
 * Renders and echoes a mustache template using the given locals and
 * layout. Most of the work is delegated to mustache_template()
 *
 * @param string $path name of the .mustache file to render
...
function render_layout($path, $locals = array(), $layout = null) {
	...
	if ($layout !== false) {
		...
		if ($layout == null) {
			$layout = config('mustache.layout') ?: config('dispatch.layout');
			$layout = ($layout == null) ? 'layout' : $layout;
		}

		// render the layout while plugging in $content
		return render($layout, array('content' => $content) + $locals);
...
/**
 * Renders a mustache template specified by $path, using scope variables
 ...
 * by config('mustache.views') and config('mustache.partials'), respectively.
 * Settings for 'mustache.charset', 'mustache.strict_callables',
 * and 'mustache.layout' are also pulled from config(), if present.
 *
 * @param string $path name of the .mustache file to render
 * @param array $locals scope variables to load inside the template
...
function render($path, $locals = array()) {
	...
	if (!$engine) {
		...
		$views_path = config('mustache.views') ?: config('dispatch.views');

		$opts = array(
			'loader' => new Mustache_Loader_FilesystemLoader($views_path),
			'partials_loader' => new Mustache_Loader_FilesystemLoader($views_path),
			'strict_callables' => config('mustache.strict_callables') ?: true,
			'charset' => config('mustache.charset') ?: 'UTF-8',
		);

		if ($cache_path = config('mustache.cache'))
			$opts['cache'] = $cache_path;

		$engine = new Mustache_Engine($opts);

		$engine->addHelper('verbatim', function($text) {
	...
	$data_out = $engine->render($path, $locals);
...
function partial($path, $locals = array(), $layout = null) {

	echo render_layout($path, $locals, $layout);

}

function view_no_layout($path, $locals= array()){
	partial($path, $locals, false);
}
